    ...
    var map_opts = {
        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                if (parseInt(code) <= 40) {
                    for (var i = 0; i < air_area_name.length; i++) {
                        if (air_area_name[i].id == code) {
                            area_text = air_area_name[i].name;
                            break;
                        }
                    }
                }
        
                var value_text = ["一", "二", "三"];

                return area_text + "<br />" + create_date + current_map_value_name + ": " + value_text[value - 1] + "級防制區";
            }
        },
        ...
        for (var i = 0; i < co_map.length; i++) {
            if (co_map[i].level == "Area") {
                ...
                for (var k = 0; k < co_map[i].value.length; k++) {
                    ...
                    var mm = parseInt(year.split("_")[1]);
                    if ((yy == sy) && (mm == sm)) {
                        var area_data = $.extend(true, [], co_map[i].value[k].data);
                        for (var g = 0; g < area_data.length; g++) {
                            geo_data.push(area_data[g]);
                        }
                        has = true;
                        break; //exit for [i]
                    }
...
var air_area_name = new Array();
var load_air_area_name = false;

function show_area(yy) {
    
    //刪除其它的區
    var geo_c2 = air_area;
    for (var i = geo_c2.features.length - 1; i >= 0; i--) {
        var code = geo_c2.features[i].properties.id;
        if (parseInt(code) <= 50) {
            if (load_air_area_name == false) {
                air_area_name.push({
                    id: geo_c2.features[i].properties.id,
                    name: geo_c2.features[i].properties.name
                });
            }
        }
    }

    load_air_area_name = true;
