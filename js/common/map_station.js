...
function show_station() {
    var s = new Array();
    var yy = current_sy - 1911;

    for (var i = 0; i < area_code.length; i++) {
        var data = area_code[i];
        if (data.level == "Station" && parseFloat(data.year) <= yy) {
            s.push({
                name: data.text,
                x: parseFloat(data.x),
                y: parseFloat(data.y) * -1,
                marker: {
                    fillColor: "#00CCFF",
                    lineColor: "#000000",
                    lineWidth: 1,
        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                return area_text + "<br />" + create_date + 
                    ((map_legend_sub_text == "") ? current_map_value_name : map_legend_sub_text)
                    + ": " + value;
            }
        },
...
function change_year_data(sy, sm, chart) {
    ...
    if (map_object) {
        ...
        var title = "" + area_text + "  " + current_site_name + " " + (sy - 1911).toString() + " 年 ";
