...
function change_area_level(action) {
    var area_level = ["ALL", "Air", "Station"];
    var index = -1;
...
function set_color_class(value, index, is_percent, fixed) {
    ...
    for (var i = 0; i <= value.length; i++) {
        ...
        if (i == 0) {
            c.to = value[0];
            if (is_percent == true) {
                c.name = "< " + (value[0] * 100).toFixed(0) + " %"
            }
            else {
                c.name = "< " + (value[0]).toFixed(f);
            }
        }
        if (i == value.length) {
            c.from = value[value.length - 1];
            if (is_percent == true) {
                c.name = "> " + (value[value.length - 1] * 100).toFixed(0) + " %"
            }
            else {
                c.name = "> " + (value[value.length - 1]).toFixed(f)
            }
        }
        if (i >= 1 && i < value.length) {
            ...
            if (is_percent == true) {
                var v1 = (value[i - 1] * 100).toFixed(0) + " %";
                var v2 = (value[i] * 100).toFixed(0) + " %";
                c.name = v1 + " - " + v2;
            }
            else {
                var v1 = (value[i - 1]).toFixed(f);
                var v2 = (value[i]).toFixed(f);
                c.name = v1 + " - " + v2;
            }
        }
        color.push(c);
    ...
    //if (is_percent == true) {
    //    c.name = "" + (i * 100).toFixed(1) + "% - " + ((i + n) * 100).toFixed(1) + "%"
    //}
...
function show_station() {
    ...
    for (var i = 0; i < co_chart.length; i++) {
        ...
        var tw97 = latlng_to_TW97(data.x, data.y);

        if ((data.level == "Station") && (data.BasinId == BasinId)) {
            var s_id = data.area;
            var click_func = (function (s_id) {
                return function () {
                    change_chart_title("Station", s_id);
                    change_chart_data("Station", s_id);
                };
            })(s_id);
...
function show_map(area, container, opts) {
    ...
    function get_air_geo(mapKey) {
        ...
        var geo_c2 = $.extend(true, {}, geo_all);

        //刪除其它的區
        for (var i = geo_c2.features.length - 1; i >= 0; i--) {
            var code = geo_c2.features[i].properties.sn;
            var ck = (code == mapKey);
            if (ck == false) {
        ...
        legend: {
            ...
            title: {
                text: current_map_value_name + " <br>"
                    + ((map_legend_sub_text == null) ? "" : map_legend_sub_text)
            }
        },
...
function change_year_data(sy, sm, chart) {
    ...
    if (map_object) {
        ...
        map_object.setTitle(
            {
                text: "" + area_text + "  " + current_site_name + " " + (sy - 1911).toString() + " 年 ",
                style: {
                    fontSize: "14px"
                }
            }, {
