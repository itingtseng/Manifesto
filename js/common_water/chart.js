...
var line_color = ["#0c3c53", "#8e0f0c", "#1c5c2f", "#8e4219"];
var chart_data_ary = [];
...
function change_chart_data(level, code) {
    ...
    if (chart_data_ary.length == 0) {
        chart_data_ary.push(co_chart)
    }

    if (stock_chart) {
        var find = false;
        for (var k = 0; k < chart_data_ary.length; k++) {
            var chart_data = chart_data_ary[k];
        ...
        if (first_load == false) {
            ...
            stock_chart.xAxis[0].setExtremes(rang.min, rang.max - 0, true);
        }
        ...
        tooltip: {
            ...
            formatter: function () {
                ...
                var str_d = d.getFullYear() - 1911 + "年 "
                    + ((data_unit == "month") ? (d.getMonth() + 1) + "月" : "")
