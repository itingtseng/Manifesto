...
function get_station_value(yy, mm, code) {
    for (var i = 0; i < co_chart.length; i++) {
        ...
        if ((data.level == "Station") && (data.area == code)) {
            ...
            var time_value = data.time_value;
            for (var k = 0; k < time_value.length; k++) {
                var d = new Date(time_value[k][0]);
                var dy = d.getFullYear();
                var dm = d.getMonth() + 1;
                if ((yy == dy) && (dm == mm)) {
                    return time_value[k][1];
                }
            }
        }
    }
...
function show_station() {
    ...
    for (var i = 0; i < point_list.length; i++) {
        ...
        var data = point_list[i];
        var value = get_station_value(current_sy, current_sm, data.name);
...
function show_map(area, container, opts) {
    ...
    var map_opts = {
        ...
        chart: {
            ...
            events: {
                click: function(){
                    change_chart_title("ALL", "ALL");
                    change_chart_data("ALL", "ALL");
                },
                drillup: function () {
