Partial Class create_js_create_js_co
    ...
    Private Sub create_all()
        ...
        Dim path As String = Server.MapPath("../js/gasoline_use/co_map.js")
    ...
    Private Sub create_data()
        ...
        js_ary.Add(get_data_json(dt_site, sd, ed, "ALL", {})) '全台

        '空品區
        For Each k In area_c.Keys
            js_ary.Add(get_data_json(dt_site, sd, ed, k, area_c(k)))
        Next

        '縣市
        For Each city In area_c
            For Each c In city.Value
                js_ary.Add(get_data_json(dt_site, sd, ed, c, {c})) '全台
            Next
        Next
        ...
        Dim path As String = Server.MapPath("../js/gasoline_use/co_chart.js")
        Dim fn As New System.IO.FileStream(path, IO.FileMode.Create)
    ...
    Private Function get_data_json(data() As WhitePaper.Table.AirQualityMonth,
                             StartDate As DateTime, EndDate As DateTime, area_code As String, area() As String) As String
        ...
        For i As Integer = 0 To m_count
            ...
            Dim s As String = String.Format("[{0},{1}]",
                                             Me.GetTimeText(month.Year, month.Month), value.ToString("#,##0.00"))
            js_time.Add(s)
        Next

        Dim j2 As String = String.Join(",", js_time.ToArray())
        Dim json As String = String.Format("{{""area"":""{0}"", ""time_value"":[{1}] }}",
                                           area_code, j2)
    ...
    Protected Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Dim path As String = Server.MapPath("../js/gasoline_use/co_oil.js")
        Dim fn As New System.IO.FileStream(path, IO.FileMode.Create)
        Dim sw As New System.IO.StreamWriter(fn, System.Text.Encoding.UTF8)
        sw.Write(result)
        sw.Close()
