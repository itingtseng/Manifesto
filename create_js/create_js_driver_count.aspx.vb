Partial Class create_js_create_js_co
    ...
    Private Sub create_all()
        ...
        Dim dt_site = (From t In dc.AirQualityMonth
                       ...
                       Where t.ITEMNAME = "一氧化碳" And t.MONTHLYVALUE IsNot Nothing _
                           And t2.SITETYPE <> "交通"
        ...
        Dim path As String = Server.MapPath("../js/air/co_map.js")
    ...
    Private Sub create_data(table As String)
        ...
        Dim ed As New DateTime(2015, 12, 1)
        ...
        Dim path As String = Server.MapPath("../js/driver/" & table & "_count.js")
        Dim fn As New System.IO.FileStream(path, IO.FileMode.Create)
    ...
    Protected Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Dim dt_drive = (From t In dc.COemission).ToArray()

        Dim j2 As String = Me.GetDriveJson("Scootor", dt_drive, sd, ed)
        Dim j3 As String = Me.GetDriveJson("Car", dt_drive, sd, ed)
        Dim j4 As String = Me.GetDriveJson("Diesel", dt_drive, sd, ed)
        ...
        Dim path As String = Server.MapPath("../js/air/co_drive.js")
        Dim fn As New System.IO.FileStream(path, IO.FileMode.Create)
        Dim sw As New System.IO.StreamWriter(fn, System.Text.Encoding.UTF8)
        sw.Write(result)
        sw.Close()
    ...
    Private Function GetDriveJson(type As String, data() As WhitePaper.Table.COemission,
                 StartDate As DateTime, EndDate As DateTime) As String

        Dim dt = (From t In data Where t.TYPE = type).First()
        Dim dt_type As System.Type = dt.GetType
        ...
        For Each prop In dt_type.GetProperties
            Dim n As String = prop.Name
            Dim v As String = prop.GetValue(dt)

            If regex.IsMatch(n) Then
                Dim y As Integer = n.Substring(1, 4)
